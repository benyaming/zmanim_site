...
- **Repository variable** `NEXT_PUBLIC_GOOGLE_CLIENT_ID` *(optional)* — a Google OAuth **web** client id enabling Sign in with Google. Uses the ID-token flow (identity only, no scopes, no Drive); the site's origin must be an Authorized JavaScript origin on the client. Public by design (an OAuth client id is not a secret; there is no client secret in this flow). Needs `NEXT_PUBLIC_TG_BOT_API_URL` set too — the bot verifies the token and stores the settings — and the bot process must carry the **same** id in `GOOGLE_CLIENT_ID`. Unset = that section is hidden.
