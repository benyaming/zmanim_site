...
// just works on any network — plus broad private ranges as a fallback, and
// ngrok tunnels (used to test the Telegram Mini App, which needs https).
function localDevOrigins(): string[] {
  const ips = Object.values(networkInterfaces())
    .flat()
    .filter((i): i is NetworkInterfaceInfo => !!i && i.family === 'IPv4' && !i.internal)
    .map((i) => i.address);
  return [...new Set([...ips, '192.168.*.*', '10.*.*.*', '172.*.*.*', '*.ngrok-free.app'])];
}
