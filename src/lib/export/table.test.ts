...
import { zmanimTableToCsv } from './csv';
...
describe('zmanimTableToCsv', () => {
  ...
  const csv = zmanimTableToCsv({
    table,
    fixedHeaders: ['Date', 'Day', 'Hebrew date', 'Holiday'],
    dayColumns: [{ key: 'dafYomi', header: 'Daf Yomi' }],
    zmanHeaders: ['Sunrise', 'Sunset'],
    footer: 'Generated by example.com',
  });
  ...
  it('quotes cells that contain a comma or quote', () => {
    const quoted = zmanimTableToCsv({
      table: { keys: ['x'], rows: [] },
      fixedHeaders: ['a,b', 'c"d'],
      dayColumns: [],
      zmanHeaders: [],
      footer: 'plain',
    });
