...
export const RELEASES: readonly Release[] = [
  {
    ...
    notes: {
      en: [
        'The Tools menu now opens a catalog of tools and ships the first two. "Export calendar" downloads print-ready PDF pages of the month grid for any month range — civil or Hebrew months, in color, black & white or dark style, with a text-size choice and a live preview. "Export zmanim" downloads a table of the zmanim you pick for any date range as an Excel file or a PDF. Both exports can be generated in any app language, independent of the interface language.',
        'Both exporters can export for the current location or any saved one, with per-export elevation and lehumra options. The zmanim table can also include parsha & special-Shabbat, candle-lighting & havdala, and fast-time columns, and every exported page and spreadsheet carries a small "generated by" note. In the calendar settings, the havdala opinion now sits right next to the candle-lighting setting.',
        'Dialogs now use the width of larger screens: the settings and export menus lay out in columns instead of one narrow scrolling stack.',
      ],
      he: [
        'תפריט הכלים נפתח כעת כקטלוג כלים וכולל את שני הראשונים. ״ייצוא לוח שנה״ מוריד עמודי PDF מוכנים להדפסה של לוח החודש לכל טווח חודשים — חודשים אזרחיים או עבריים, בצבע, בשחור־לבן או בסגנון כהה, עם בחירת גודל טקסט ותצוגה מקדימה. ״ייצוא זמנים״ מוריד טבלה של הזמנים שבחרתם לכל טווח תאריכים כקובץ Excel או PDF. את שני הייצואים אפשר להפיק בכל שפת אפליקציה, ללא תלות בשפת הממשק.',
        'שני הכלים יכולים לייצא עבור המיקום הנוכחי או כל מיקום שמור, עם אפשרויות גובה ולחומרה לכל ייצוא. טבלת הזמנים יכולה לכלול גם עמודות פרשה ושבת מיוחדת, הדלקת נרות והבדלה וזמני צום, וכל עמוד וגיליון מיוצאים נושאים הערת ״הופק על ידי״ קטנה. בהגדרות הלוח, שיטת ההבדלה יושבת כעת ליד הגדרת הדלקת הנרות.',
        'חלונות הדו־שיח מנצלים כעת את רוחב המסכים הגדולים: תפריטי ההגדרות והייצוא נפרסים בעמודות במקום ערימה צרה וגוללת.',
      ],
      ru: [
        'Меню инструментов теперь открывается каталогом инструментов, и в нём появились первые два. «Экспорт календаря» скачивает готовые к печати PDF-страницы календарной сетки за любой диапазон месяцев — гражданские или еврейские месяцы, в цвете, в чёрно-белом или в тёмном стиле, с выбором размера текста и предпросмотром. «Экспорт зманим» скачивает таблицу выбранных зманим за любой диапазон дат — файлом Excel или PDF. Оба экспорта можно создавать на любом языке приложения, независимо от языка интерфейса.',
        'Оба инструмента могут экспортировать для текущего места или любого сохранённого, с настройками высоты и ле-хумра для каждого экспорта. Таблица зманим может также включать колонки недельной главы и особого шаббата, зажигания свечей и авдалы, а также времён постов, а каждая экспортированная страница и таблица несёт небольшую пометку «создано на». В настройках календаря мнение для авдалы теперь находится рядом с настройкой зажигания свечей.',
        'Диалоги теперь используют ширину больших экранов: меню настроек и экспорта раскладываются в колонки вместо узкого прокручиваемого столбца.',
      ],
    },
  },
  {
