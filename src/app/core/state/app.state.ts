...
import {FetchLocationFromFreegeoip, FetchLocationFromNavigator, FetchZmanim, SetLocationManually,} from "./app.actions";
...
export class AppState {
  ...
  fetchLocationFromNavigator(ctx: StateContext<AppStateModel>): Observable<any> {
    return new Observable<GeolocationPosition>(observer => {
      ...
      tap(res => {
        ...
        if (!place) {
          console.error(`received no places for given coords from navigator: lat [${res.query[1]}], lng [${res.query[0]}]`);
          return;
        }
  ...
  fetchLocationFromFreegeoip(ctx: StateContext<AppStateModel>): Observable<any> {
  ...
  setLocationManually(ctx: StateContext<AppStateModel>, {location}: SetLocationManually): void {
    this.setLocation(ctx, {...location, source: 'manual'});
  }

  @Action(FetchZmanim)
  fetchZmanim(ctx: StateContext<AppStateModel>, {form}: FetchZmanim): Observable<any> | undefined {
  ...
  private setLocation(ctx: StateContext<AppStateModel>, location: LocationModel): void {
